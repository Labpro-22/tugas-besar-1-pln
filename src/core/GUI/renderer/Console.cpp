...
Dice::Dice(const sf::Font& f) : font(f) {}
...
void Dice::draw(sf::RenderWindow& win, float x, float y, const DiceInfo& dice) {
    float gap  = 12.f;
    float dieW = (float)DICE_SZ;
    float spin = dice.rolling ? spinAngle : 0.f;

    drawDie(win, x, y, dice.d1, dice.rolling, spin);
    drawDie(win, x+dieW+gap, y, dice.d2, dice.rolling, spin);
    ...
    if (!dice.rolling) {
        ...
        sf::Text sut;
        sut.setFont(font);
        sut.setString("= " + std::to_string(sum));
        sut.setCharacterSize(15);
        sut.setFillColor({200,200,220});
        float totalW = dieW * 2.f + gap;
        auto b = sut.getLocalBounds();
        sut.setPosition(x + totalW/2.f - b.width/2.f - b.left,y + dieW + 5.f);
        win.draw(sut);
    }
}
