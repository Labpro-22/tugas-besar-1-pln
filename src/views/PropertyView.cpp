...
void PropertyView::outputProperty() const{
    ...
    if (pt == nullptr || pt->getProperty() == nullptr) {
        ...
        std::cout << "+" << std::string(WIDTH, '=') << "+\n";
        std::cout << "|" << std::left << std::setw(COLON_WIDTH) << " Harga Beli" << std::left << std::setw(WIDTH-COLON_WIDTH) << ": M" + std::to_string( pr->getPrice())<<"|\n";
        std::cout << "|" << std::left << std::setw(COLON_WIDTH) << " Nilai Gadai" << std::left << std::setw(WIDTH-COLON_WIDTH) << ": M" + std::to_string( pr->getMortgageValue())<<"|\n";
        std::cout << "+" <<std::string(WIDTH, '-') << "+\n";
        if(auto sp = dynamic_cast<StreetProperty*>(pr)){
            ...
            for(int i = 0; i < (int)rent.size(); i++){
                std::cout << "|" << std::left << std::setw(COLON_WIDTH) << (" Sewa (" + std::to_string(i) + " Railroad dimiliki)") << std::left << std::setw(WIDTH-COLON_WIDTH) <<
            ...
            for(int i = 0; i < (int)rent.size(); i++){
                std::cout << "|" << std::left << std::setw(COLON_WIDTH) << (" Sewa (" + std::to_string(i) + " Utility dimiliki)") << std::left << std::setw(WIDTH-COLON_WIDTH) <<
                (": " + std::to_string(rent[i])) << "|\n|" << std::right << std::setw(WIDTH) <<  "kali lipat jumlah hasil lemparan dadu|" << "\n";
            }
        }
        std::cout << "+" << std::string(WIDTH, '=') << "+\n";
        std::cout << "|";
        ...
        {
        case OWNED:
            std::cout << std::left << std::setw(WIDTH) << " Status : OWNED (Pemain" + pr->getOwner()->getUsername()+")";
            break;
        case MORTGAGED:
            std::cout << std::left << std::setw(WIDTH) << " Status : MORTGAGED [M]";
            std::cout << "|" <<std::left << std::setw(WIDTH) << " Harga tebus : M" + std::to_string(pr->redemptionPrice()) << "\n";
            break;
        case BANK:
            std::cout << std::left << std::setw(WIDTH) << " Status : BANK";
            break;
        default:
            break;
        }
    ...
    if(p.getProperties().empty()){
        ...
        for(auto pr : playerProperties){
            if(auto sp = dynamic_cast<StreetProperty*>(pr)){
                ...
                std::cout << "  - " << std::left << std::setw(SPACE)  << (sp->getName() +  " (" + sp->getCode() +")");
                if(sp->hasHotel()){
                    std::cout << "Hotel  "; 
                }
...
void PropertyView::outputRent(Property &pr, Player &p)const{
    if(pr.isMortgaged()){
        ...
        std::cout << std::left << std::setw(width) << "Kondisi" << ": ";
        if(auto sp = dynamic_cast<StreetProperty*>(&pr)){
            std::cout << sp->getHouseCount() << " rumah\n"; 
        }
        else if(auto up = dynamic_cast<UtilityProperty*>(&pr)){
